...
export const markAttendance = async (req, res) => {
  let { studentId, status, mood, moodScore } = req.body;
  ...
  try {
    ...
    const validMoodScores = student.attendance
      .filter(a => typeof a.moodScore === 'number')
      .map(a => a.moodScore);

    if (validMoodScores.length > 0) {
      const moodSum = validMoodScores.reduce((sum, score) => sum + score, 0);
      student.moodAvg = Number((moodSum / validMoodScores.length).toFixed(2));
      await student.save();
    }
